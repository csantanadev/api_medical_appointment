...
export class FreeScheduleUseCase {
    ...
    async execute(data: FreeScheduleRequest): Promise<FreeScheduleResponse> {
        ...
        if (!data.date) {
            throw new CustomError('You need to select a date.', StatusCodes.BAD_REQUEST);
        }
        ...
        while (timeNow <= endAt) {
            ...
            const existsAppointment = appointmentsByDoctorAndDate.find((app) => formatDate(app.date, 'HH:mm') === timeNow);
            ...
            timeNow = dayjs(data.date + timeNow).add(duration, 'minute').format('HH:mm'); // add 30 min a cada volta
        }
