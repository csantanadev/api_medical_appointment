...
export class DoctorSchedule {
    ...
    private constructor(props: DoctorScheduleProps) {
        ...
        if (!props.schedules) {
            throw new CustomError('Invalid schedules.', StatusCodes.UNPROCESSABLE_ENTITY, 'PARAMETER_REQUIRED');
        }
        // validator
    ...
    private validateSchedules(schedules: Schedules[]) {
        ...
        const hasUniqueValue = new Set(schedules.map(schedule => schedule.dayOfWeek));
        ...
        schedules.forEach(schedule => {
            ...
            if (!validateTime(schedule.startAt)) {
                throw new CustomError('Invalid StartAt.', StatusCodes.UNPROCESSABLE_ENTITY);
            }
            if (!validateTime(schedule.endAt)) {
                throw new CustomError('Invalid EndAt.', StatusCodes.UNPROCESSABLE_ENTITY);
            }
            if (!compareEndTimeIsAfter(schedule.startAt, schedule.endAt)) {
                throw new CustomError('End time cannot be earlier than start time.', StatusCodes.UNPROCESSABLE_ENTITY);
            }
